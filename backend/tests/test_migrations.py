...
def test_routing_record_migration_backfills_legacy_client_key_and_indexes(
    tmp_path: Path,
) -> None:
    ...
    with sqlite3.connect(database) as connection:
        connection.executescript(
            ...
            INSERT INTO requests (date, model, api_key_id)
            VALUES ('2026-08-08', 'legacy-model', 7);
            """
        )
        connection.commit()
...
def test_request_retention_migration_defaults_off_and_is_idempotent(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(database) as connection:
        config_row = connection.execute(
            "SELECT enabled, retention_days FROM request_retention_settings WHERE id = 1"
        ).fetchone()
        lock_table = connection.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='maintenance_locks'"
        ).fetchone()
        retention_index = connection.execute(
            "SELECT 1 FROM sqlite_master WHERE type='index' "
            "AND name='idx_requests_retention_date_id'"
        ).fetchone()
    assert config_row == (0, 30)
    assert lock_table == (1,)
    assert retention_index == (1,)


def test_client_key_name_snapshot_migration_backfills_existing_requests(
    tmp_path: Path,
) -> None:
    database = tmp_path / "client-key-name.db"
    with sqlite3.connect(database) as connection:
        connection.executescript(
            """
            ...
            INSERT INTO api_keys (id, name) VALUES (7, '保留名称');
            INSERT INTO requests (date, model, api_key_id, client_api_key_id)
            VALUES ('2026-08-09', 'fixture-model', 7, 7);
            """
        )
    ...
    with sqlite3.connect(database) as connection:
        snapshot = connection.execute(
            "SELECT client_api_key_name FROM requests WHERE id = 1"
        ).fetchone()[0]
    assert snapshot == "保留名称"
